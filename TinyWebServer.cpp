...
// By default Debugging to the Serl console is OFF.
...
TinyWebServer::TinyWebServer(PathHandler handlers[],
	...
	client_(EthernetClient(MAX_SOCK_NUM))
{
	if (headers) {
		int i = 0;
		for (; headers[i] && i < MAX_HEADERS - 1; i++) {
			headers_[i].header = headers[i];
			headers_[i].value[0] = 0;
		}
		headers_[i].header = NULL;
	}
}
...
void TinyWebServer::process() {
	...
	char *saveptr;
	char* request_type_str = strtok_r(buffer, " \t", &saveptr);
	request_type_ = UNKNOWN_REQUEST;
...
char* TinyWebServer::decode_url_encoded(char* s) {
	if (!s) {
		return NULL;
	}
	char* r = s; // Will overwrite input string
	char* r2 = r;
	char* p = s;
	while (*s && (p = strchr(s, '%'))) {
		if (p - s) {
...
char* TinyWebServer::get_file_from_path(char* path) {
	// Obtain the last path component.
	char* encoded_fname = strrchr(path, '/');
	if (!encoded_fname) {
		return NULL;
	}
	else {
